...
void henTrace(String Function() message) {
  assert(() {
    debugPrint(message());
    return true;
  }());
...
class TrailAttribution {
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      final sdk = AppsflyerSdk(
        AppsFlyerOptions(
          afDevKey: CoopRelayConfig.appsFlyerKey,
          appId: CoopRelayConfig.iosStoreId,
      ...
      henTrace(() => '[HYS.TRAIL] initialization failed: $error');
      _completeEmpty();
    }
  }
  ...
  Future<void> _acceptInstall(dynamic raw) async {
    try {
      ...
      // servers (e.g. an ad-blocking VPN blackholes *.appsflyersdk.com).
      // Never merge that error map into the payload.
      final failed = status == 'failure' ||
          (received['af_status'] == null && received.containsKey('status'));
      henTrace(
        () => '[HYS.TRAIL] conversion status=$status '
      ...
      if (failed) {
        ...
        await Future<void>.delayed(
          const Duration(seconds: CoopRelayConfig.organicRecheckSeconds),
        );
      ...
      henTrace(() => '[HYS.TRAIL] conversion parse error: $error');
      _install = <String, dynamic>{};
    } finally {
  ...
  Future<Map<String, dynamic>?> _fetchGcd() async {
    ...
    try {
      ...
      final base = CoopRelayConfig.gcdBase;
      final sep = base.contains('?') ? '&' : '?';
      final uri = Uri.parse(
        '$base${sep}app_id=${CoopRelayConfig.iosStoreId}&device_id=$uid',
      );
      final response = await _agent
          .get(
            uri,
            headers: <String, String>{
              'Authorization': 'Bearer ${CoopRelayConfig.appsFlyerKey}',
            },
          )
  ...
  Future<Map<String, dynamic>> compose({
    ...
    body['bundle_id'] = CoopRelayConfig.bundleId;
    body['os'] = 'iOS';
    body['store_id'] = CoopRelayConfig.storeToken;
    body['locale'] = locale;
    if (pushToken != null &&
        pushToken.isNotEmpty &&
        CoopRelayConfig.firebaseProjectNumber.isNotEmpty) {
      body['push_token'] = pushToken;
      body['firebase_project_id'] = CoopRelayConfig.firebaseProjectNumber;
    }
    ...
    henTrace(() => '[HYS.TRAIL] payload ${jsonEncode(body)}');
    return body;
  }
