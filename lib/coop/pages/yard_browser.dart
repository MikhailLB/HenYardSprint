...
class _YardBrowserState extends State<YardBrowser> with WidgetsBindingObserver {
  ...
  void didChangeMetrics() {
    ...
    if (!rotated) return;
    _enterImmersive();
    // WKWebView keeps the pre-rotation viewport width for a few hundred ms, so
    // the site renders at the wrong width right after the flip. Kick a
    // resize/orientationchange several times as the native frame settles.
  ...
  void _installEdgeGuard() {
    _controller.runJavaScript(r'''
(() => {
  const root = window;
  if (root.__hysEdgeGuard) return;
  root.__hysEdgeGuard = true;
  const marker = 'hys-edge-sheet';
  const rules = [
    ':root{',
    ...
    '}',
    // Lock the document edges: no rubber-band overscroll that would reveal
    // the black scaffold above/below the site. Does not touch site layout.
  ...
  const schedule = () => {
    root.setTimeout(applyEdges, 170);
    root.setTimeout(applyEdges, 640);
  };
  ...
  applyEdges();
  root.setInterval(applyEdges, 2900);
})();
''');
  }

  /// Locks the page at 1:1 scale — no pinch / double-tap / gesture zoom, so the
  /// offer behaves like a native screen. Idempotent + re-asserts the viewport
  /// on SPA navigations.
  void _installScaleLock() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__hysScaleLock) return;
  window.__hysScaleLock = true;
  const lockViewport = () => {
    const host = document.head || document.documentElement;
    if (!host) return;
...
''');
  ...
  /// Kills the grey/black translucent box WKWebView paints on every tap and the
  /// long-press callout, so tapping feels native. Inputs stay selectable.
  void _installTapClean() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__hysTapClean) return;
  window.__hysTapClean = true;
  const style = document.createElement('style');
  style.id = 'hys-tap-clean';
  style.textContent =
...
''');
  ...
  void _installInputRaise() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__hysInputRaise) return;
  window.__hysInputRaise = true;
...
''');
  ...
  void _installFontFix() {
    if (!Platform.isIOS) return;
    _controller.runJavaScript(r'''
(() => {
  if (window.__hysFontFix) return;
  window.__hysFontFix = true;
  const style = document.createElement('style');
  style.textContent =
...
''');
  ...
  void _installInlineVid() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__hysInlineVid) return;
  window.__hysInlineVid = true;
  const awaken = (video) => {
...
''');
  ...
  /// (it never creates a second window), so payment / redirect links — e.g. the
  /// BDM offer — do nothing when tapped. Route those into the top frame: http(s)
  /// then loads in-place, and custom schemes fall through to onNavigationRequest
  /// which launches the external app.
  void _installNewWindow() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__hysNewWindow) return;
  window.__hysNewWindow = true;
  ...
  window.open = function(url, name, features) {
    ...
    return nativeOpen ? nativeOpen.call(window, url, name, features) : window;
  };
  // Neutralize target=_blank/_new on the way down so the click navigates the
  // current frame instead of asking WKWebView for a new (dropped) window.
